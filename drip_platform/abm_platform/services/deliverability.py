...
def update_reputation(db: Session, domain: str) -> p11.DomainHealth:
    ...
    bounces = sum(1 for e in events if normalize(e.event_type) in ("soft_bounce", "hard_bounce"))
    complaints = sum(1 for e in events if normalize(e.event_type) == "complaint")
...
def rate_card(db: Session, campaign_id: str | None = None) -> dict:
    ...
    by: dict[str, set] = {}
    for e in events:
        by.setdefault(normalize(e.event_type), set()).add(e.message_id)
    delivered = len(by.get("delivered", set()))
    opened = len(by.get("open", set()))
    clicked = len(by.get("click", set()))
    bounced = len(by.get("soft_bounce", set()) | by.get("hard_bounce", set()))
    spam = len(by.get("complaint", set()))
    unsub = len(by.get("unsubscribe", set()))
