...
PROMOTION_RULE_VERSION = "deterministic-v1"
...
class Pipeline:
    ...
    def add_account(self, account_id: str, canonical_name: str, aliases=None, domains=None, tickers=None) -> None:
        self.con.execute(
            "INSERT OR REPLACE INTO accounts(id, canonical_name, aliases_json, domains_json, tickers_json) VALUES(?,?,?,?,?)",
    ...
    def resolve_review(self, review_id: str, resolution: str, account_id: str | None = None) -> dict:
        ...
        if resolution == "approve":
            ...
            action = int(confidence >= .80 and relevance >= .70 and signal_type in {"tender", "regulatory"})
            self.con.execute(
                ...
                (uid(), review["observation_id"], account_id, signal_type, direction, urgency, review["title"], review["body"],
    ...
    def ingest_item(self, source_id: str, item: dict, raw_payload: str, observed_at: datetime) -> str:
        ...
        if status == "corroborating":
            ...
            self.con.execute("""UPDATE signals SET confidence=MIN(0.95,coverage_cap,confidence+0.05),
                              scoring_eligible=CASE WHEN MIN(0.95,coverage_cap,confidence+0.05)>=0.60 AND relevance_score>=0.50
                              AND expires_at>? THEN 1 ELSE scoring_eligible END WHERE id=?""",
                             (iso(observed_at), corroborates_signal_id))
            self.con.commit()
        ...
        confidence = min(0.95, coverage, 0.35 * attribution.confidence + 0.30 * source_score + 0.25 * relevance + 0.10 * freshness)
        scoring_eligible = int(confidence >= 0.60 and relevance >= 0.50 and expires > observed_at)
        action_eligible = int(confidence >= 0.80 and relevance >= 0.70 and signal_type in {"tender", "regulatory"})
        signal_id = uid()
        self.con.execute(
            """INSERT INTO signals(id,observation_id,account_id,signal_type,direction,urgency,title,summary,product_match,event_at,
               decay_category,expires_at,relevance_score,source_score,confidence,coverage_cap,scoring_eligible,action_eligible,
