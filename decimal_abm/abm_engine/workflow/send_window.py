"""
abm_engine/workflow/send_window.py
────────────────────────────────────
KSA send-window guard.
...
Nothing in the live codebase enforced this before Phase 1 (confirmed by
grepping the whole package for "friday"/"blackout"/"ramadan" — zero hits
outside this file). Every send path (scheduler cron + any manual "send now")
should call `is_within_send_window()` before actually dispatching mail, and
skip — not error — when it returns False. Skipped sends stay APPROVED and
unsent; they go out on the next tick that falls inside the window.

KSA's weekend is Friday–Saturday (moved from Thu–Fri in 2013). Business hours
default to 08:00–18:00 Asia/Riyadh, Sunday–Thursday. All of this is
env-configurable so it can be tightened/loosened without a code change.
"""
...
def _blackout_dates() -> set[date]:
    """
    Optional static blackout dates (Ramadan, Eid, National Day, ...) loaded
    from a JSON file of "YYYY-MM-DD" strings. File is optional — if
    RAMADAN_BLACKOUT_DATES_FILE isn't set or doesn't exist, this is a no-op.
    Keeping it file-based (not hardcoded) because Hijri holiday dates shift
    every Gregorian year and must not require a code change to update.
    """
    ...
    try:
        ...
        logger.warning("Could not load blackout dates from {}: {}", path, e)
        return set()
